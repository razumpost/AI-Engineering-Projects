...
def _skus_from_kuzu_for_deal(conn: kuzu.Connection, task_ids: list[int], limit_items_per_snap: int) -> set[str]:
    skus: set[str] = set()

    for tid in task_ids:
        task_nodes = _find_task_node_ids(conn, tid, limit=10)
        for tn in task_nodes:
            snap_ids = _neighbors_by_edge(conn, tn, want_type="Snapshot", limit=80)
            for sid in snap_ids:
...
def main() -> int:
    ...
    ap.add_argument("--limit-items-per-snapshot", type=int, default=5000)
    args = ap.parse_args()
