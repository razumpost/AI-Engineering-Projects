...
def seed_spec_from_role_candidates(
    *,
    ...
    """
    Deterministic baseline spec from role_candidates.
    Used when LLM fails to avoid showing unrelated last_valid specs.
    """

    spec = Spec(spec_id=_new_spec_id(), project_title="СкСп", items=[])
    spec.project_summary = request_text
    spec.why_composition = [
        f"Seed: topology={topology.topology_key}",
        "Seed построен детерминированно из role_candidates (без LLM), затем дополняется зависимостями и количествами.",
    ]
