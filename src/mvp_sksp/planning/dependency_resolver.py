...
def resolve_dependencies(
    ...
    postprocess_spec() calls: resolve_dependencies(spec, source_pool, requirements, topology)
    Older code may call: resolve_dependencies(spec, source_pool)
    """
    warnings: list[str] = []

    if not list(getattr(spec, "items", []) or []):
        return ["dependency_resolver: spec.items empty; skip"]

    if requirements is None:
        return ["dependency_resolver: requirements is None; skip"]
        ...
        if cand is not None:
            ...
            if _find_existing_placeholder(spec, "display") is None:
                getattr(spec, "items").append(
                    _build_placeholder_line(spec, kind="display", title="Панель/экран для переговорной, подобрать", category="display", qty=1)
                )
    ...
    if requirements.room_type == "meeting_room" and not has_playback:
        cand = (
            _best_candidate_for_family(source_pool, "soundbar")
            or _best_candidate_for_family(source_pool, "wall_speaker")
            or _best_candidate_for_family(source_pool, "ceiling_speaker")
        )
        if cand is not None and profile == "lowz":
            getattr(spec, "items").append(
                _build_line_from_candidate(spec, cand, qty=2 if "speaker" in (cand.description or "").casefold() else 1, category="conference")
            )
        else:
            if _find_existing_placeholder(spec, "audio_lowz") is None:
                getattr(spec, "items").append(
                    _build_placeholder_line(spec, kind="audio_lowz", title="Акустика для переговорной (низкоомная), подобрать", category="conference", qty=2)
                )
    ...
    if has_delegate and not fam.get("chairman_unit"):
        ...
        if cand is not None:
            ...
            if _find_existing_placeholder(spec, "chairman") is None:
                getattr(spec, "items").append(
                    _build_placeholder_line(spec, kind="chairman", title="Пульт председателя (1 шт), подобрать", category="conference", qty=1)
                )
    ...
    if has_delegate and not (fam.get("conference_controller") or fam.get("conference_central_unit")):
        ...
        if cand is not None:
            ...
            if _find_existing_placeholder(spec, "central") is None:
                getattr(spec, "items").append(
                    _build_placeholder_line(spec, kind="central", title="Центральный блок/контроллер конференц-системы (1 шт), подобрать", category="conference", qty=1)
                )
        ...
        if not has_byod:
            ...
            if cand is not None:
                ...
                if _find_existing_placeholder(spec, "byod") is None:
                    getattr(spec, "items").append(
                        _build_placeholder_line(spec, kind="byod", title="BYOD модуль (USB-C/HDMI), подобрать", category="signal_transport", qty=1)
                    )
                warnings.append("missing_dependency: byod")
